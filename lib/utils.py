...
def get_args():
    ...
    parser.add_argument('--alpha_reversed', type=int, default=0, help='whether the alpha is reversed or not')
    return parser
parser = get_args()
...
def get_trainable_parameters(net, device=device):
...
def compute_accuracy(model, dataloader, get_confusion_matrix=False, device=device, multiloader=False):
    was_training = False
    if model.training:
        model.eval()
        was_training = True
...
def load_model(model, model_index, device=device):
    #
...
def get_dataloader(dataset, datadir, train_bs, test_bs, args, dataidxs=None, noise_level=0):
    worker_init = partial(worker_init_fn, seed=args.init_seed)
    if dataset in ('cifar10', 'cifar100'):
        ...
        train_dl = data.DataLoader(dataset=train_ds, batch_size=train_bs, drop_last=True, shuffle=True, pin_memory=True, num_workers=8, worker_init_fn=worker_init)
        test_dl = data.DataLoader(dataset=test_ds, batch_size=test_bs, shuffle=False, pin_memory=True, num_workers=8, worker_init_fn=worker_init)
