...
# class ModelFedTennessee(nn.Module):

    def __init__(self, base_model, n_classes):
        super(ModelFedTennessee, self).__init__()
        self.base_model = base_model
        if base_model == "resnet50":
            self.resnet50 = models.resnet50(pretrained=False)
            self.resnet50.fc = nn.Linear(self.resnet50.fc.in_features, n_classes)
            # self.features = nn.Sequential(*list(self.basemodel.children())[:-1])
            # num_ftrs = self.basemodel.fc.in_features
        elif base_model == "resnet18":
            self.resnet18 = models.resnet18(pretrained=False, norm_layer=self.group_norm)
    ...
    def group_norm(self, num_channels):
        num_groups = 2
    ...
    def forward(self, x):
        ...
        if self.base_model == 'resnet50':
            ...
            feature_representations.append(x)
        ...
        return x, feature_representations
